...
if page == "🆕 New Assessment":
    ...
    if st.session_state.studies_data:
        ...
        # Create detailed export (original format)
        ...
        detailed_df = df.reindex(columns=base_cols + score_cols + summary_cols)
        
        # Rename columns for detailed export
        detailed_mapping = {
        ...
        for i in range(1, 13):
            detailed_mapping[f"item_{i}"] = f"Item {i}"
        
        detailed_df = detailed_df.rename(columns=detailed_mapping)
        
        # Create compact export format (Study | Year | 1 | 2 | 3... | Total Score | Quality)
        compact_data = []
        for _, row in df.iterrows():
            compact_row = {}
            # Create study citation format
            compact_row['Study'] = f"{row['first_author']} et al., {row['year']}"
            compact_row['Year'] = row['year']
            
            # Add individual item scores (1-12)
            study_type = row.get('study_type', 'Non-comparative')
            max_items = 8 if study_type == 'Non-comparative' else 12
            
            for i in range(1, 13):
                if i <= max_items:
                    score = row.get(f'item_{i}', 'N/A')
                    compact_row[str(i)] = score if score != 'N/A' else ''
                else:
                    compact_row[str(i)] = ''  # Empty for non-applicable items
            
            compact_row['Total Score'] = row.get('total_score', 0)
            compact_row['Quality'] = row.get('quality_rating', 'Unknown')
        ...
        export_format = st.radio(
            ...
            ["📊 Detailed Format (Full Data)", "📋 Compact Format (Study | Year | 1-12 | Total | Quality)"],
            index=1  # Default to compact format
        )
        ...
        if export_format.startswith("📊"):
            ...
            # Show compact format
            st.subheader("📋 Compact Data Preview")
            st.dataframe(compact_df, use_container_width=True)
            export_df = compact_df
        ...
        with col1:
            st.markdown(create_download_link(export_df, f"MINORS_Assessment_{filename_suffix}", "excel"), unsafe_allow_html=True)
        with col2:
            st.markdown(create_download_link(export_df, f"MINORS_Assessment_{filename_suffix}", "csv"), unsafe_allow_html=True)
        
        # Show sample of compact format for reference
        if export_format.startswith("📊"):
            with st.expander("📋 Preview Compact Format"):
                st.dataframe(compact_df, use_container_width=True)
                st.caption("This format is ideal for statistical analysis and publications")
